...
- **`--library NAME` is now repeatable and accepts new sentinels.** Pass multiple `--library` flags; bare sentinels `primary` (PrimarySync), `shared` (every SharedSync zone), and `all` (every library) join named zones and `!name` exclusions in one grammar. The TOML key `[filters].libraries` takes the array form. The truncated 8-char form (`SharedSync-A1B2C3D4`) round-trips between paths and the flag. Combinations the legacy single-library pipeline cannot execute (e.g. `primary + named`, `shared` alone, `!name` exclusions) bail at startup with a clear pointer to the multi-library resolver work. Friendly aliases like `shared:Owner Name` parse but don't yet resolve from CloudKit metadata; full alias resolution lands in a follow-up. ([#215])
