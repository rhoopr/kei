...
- Added `kei sync --repair-truncated` to replace an existing recorded truncated file only after `kei reconcile` marks it and the replacement passes validation. Ordinary sync kept no-overwrite publication. ([#730](https://github.com/rhoopr/kei/pull/730))
- Added explicit capture-timestamp repair with `kei sync --refresh-metadata --repair-capture-timestamps`. It requires enabled datetime output and usable offset/checksum evidence, can overwrite camera timestamps, and retains interrupted repair receipts for retry. ([#726](https://github.com/rhoopr/kei/issues/726))
- Added `kei reset session` to clear cookies, session caches, and trust material without deleting the stored password or state database. Active account locks block reset; sleeping workers stop on reset-generation drift. ([#717](https://github.com/rhoopr/kei/pull/717), fixes [#716](https://github.com/rhoopr/kei/issues/716))

### Changed

- State databases now use schema 25, including per-path metadata evidence, verified source provenance, and content-specific reconciliation reservations. Older binaries reject newer schemas. Capture/addition dates also retain milliseconds in a representation older readers cannot decode. Back up before upgrading; a database restore does not roll back media or sidecars. See [upgrade guidance](https://github.com/rhoopr/kei/blob/v0.24.0/docs/v0.24-upgrade.md). ([#805](https://github.com/rhoopr/kei/pull/805), [#812](https://github.com/rhoopr/kei/pull/812), thanks [@te&#104;-hippo](https://github.com/teh-hippo))
- Automatic metadata-capture repair now processes up to 500 stale assets per selected library per cycle. Watch/service follows up after at most 60 seconds when clean progress leaves work remaining; stalled or failed batches use normal cadence. One-shot sync remains bounded. Status and report version `"3"` expose capture progress. ([#733](https://github.com/rhoopr/kei/pull/733), [#742](https://github.com/rhoopr/kei/issues/742))
- Sidecars gained source EXIF GPS time and speed facts and retained provider altitude precision. Native horizontal accuracy now requires matching provider/native coordinates and verified original-source bytes; historical or rewritten files can conservatively omit it. Existing native/unowned fields remain. Untouched-sidecar catch-up is deferred. ([#725](https://github.com/rhoopr/kei/issues/725), [#768](https://github.com/rhoopr/kei/pull/768), [#807](https://github.com/rhoopr/kei/pull/807), thanks [@te&#104;-hippo](https://github.com/teh-hippo))

### Fixed

- Path changes now use verified local copying with retained directory handles and durable destinations across libraries, renditions, and content generations. Old sources remain. Copies receive capture mtime and configured sidecars; valid source packets retain their custom properties. Unsafe leaves/parents, symlinks, `..`, hard-link aliases, and conflicting bytes block completion without changing the old catalogue path. Retries reuse reserved destinations. ([#810](https://github.com/rhoopr/kei/pull/810), [#811](https://github.com/rhoopr/kei/pull/811), [#812](https://github.com/rhoopr/kei/pull/812))
- Date-bound-only changes stopped creating unnecessary identity-suffixed copies. Newly selected renditions no longer count as failed moves before they are downloaded. Existing duplicate files remain untouched. ([#794](https://github.com/rhoopr/kei/pull/794), [#795](https://github.com/rhoopr/kei/issues/795), thanks [@te&#104;-hippo](https://github.com/teh-hippo))
- HEIC/HEIF/AVIF embedded XMP switched to a byte-preserving primary-image item-map writer. It preserves encoded media and auxiliary content, refuses unsupported or ambiguous layouts, and keeps file permissions. This does not repair historically damaged containers or bound whole-file memory use. ([#558](https://github.com/rhoopr/kei/issues/558), thanks [@te&#104;-hippo](https://github.com/teh-hippo))
- Sidecar updates now preserve unreadable, malformed, concurrently changed, and unowned content. Supported rewrites update or clear kei-owned fields, converge album/people membership, and retry recorded output paths independently. First-write album membership and per-rendition Live Photo catalogue facts were corrected. ([#724](https://github.com/rhoopr/kei/pull/724), [#731](https://github.com/rhoopr/kei/pull/731), [#752](https://github.com/rhoopr/kei/pull/752), [#806](https://github.com/rhoopr/kei/pull/806), [#809](https://github.com/rhoopr/kei/pull/809), thanks [@te&#104;-hippo](https://github.com/teh-hippo))
- Provider metadata edits now refresh skipped downloaded versions without another media download. Catalogue and rewrite evidence commit together. Metadata failures remain visible even in zero-download cycles; recorded local hashes distinguish intentional rewrites from damage. ([#709](https://github.com/rhoopr/kei/pull/709), [#723](https://github.com/rhoopr/kei/pull/723), thanks [@te&#104;-hippo](https://github.com/teh-hippo))
- Capture dates now use Apple's usable per-asset offset, with host-local fallback. Date-only filters compare the capture calendar date; instant-based filters retain instant semantics. Ordinary writes preserve native timestamps. Explicit refresh can recover historical capture/addition fractions and configured sidecars without moving media. ([#703](https://github.com/rhoopr/kei/issues/703), [#805](https://github.com/rhoopr/kei/pull/805), thanks [@te&#104;-hippo](https://github.com/teh-hippo))
- Stable asset identity and durable legacy ownership prevent sibling/page order from displacing a recorded owner. Malformed required provider fields and failed grouping writes preserve affected checkpoints. Policy exclusions retain targeted provider-deletion checks instead of becoming deletion evidence themselves. ([#721](https://github.com/rhoopr/kei/pull/721), [#737](https://github.com/rhoopr/kei/pull/737), [#751](https://github.com/rhoopr/kei/pull/751), [#777](https://github.com/rhoopr/kei/pull/777))
- Different-byte destination races now fail before sidecar/state finalization. Stale temporary cleanup requires durable ownership evidence rather than suffix and age alone. ([#750](https://github.com/rhoopr/kei/pull/750), [#764](https://github.com/rhoopr/kei/pull/764))
- Headless foreground login and one-shot sync now exit 3 when 2FA needs action; watch/service still waits and resumes. Password setting accepts file/command sources without a terminal. A 403 rejection of persisted state during 2FA delivery gets one clean-session retry. ([#698](https://github.com/rhoopr/kei/issues/698), [#767](https://github.com/rhoopr/kei/pull/767), thanks [@te&#104;-hippo](https://github.com/teh-hippo))
- Updated rustls to address [RUSTSEC-2026-0285](https://rustsec.org/advisories/RUSTSEC-2026-0285).

### Known limitations

- Selected smart-folder reconciliation can repeat; enabling Live Photo movies after still-only downloads can leave inconsistent stems. Untouched sidecars have no automatic output-revision sweep, and timestamp precision work remains incomplete.
- Concurrent full-enumeration passes can exceed the worker budget. Streaming reports/metrics can undercount transfer bytes and media types. HEIF embedding still amplifies memory use; no general memory ceiling is established.

See [the issue-linked limitations](https://github.com/rhoopr/kei/blob/v0.24.0/docs/v0.24-upgrade.md#known-limitations) and [validation record](https://github.com/rhoopr/kei/blob/v0.24.0/docs/v0.24-validation.md) for the accepted limitations and validation scope.
