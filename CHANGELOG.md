...
- **`kei import-existing --dry-run`.** Scans the tree, runs filename + size matching, and computes checksums the same way a real import does, but doesn't write anything to the state database. Useful before committing: a user with 50k existing photos can verify that `--folder-structure` and `--keep-unicode-in-filenames` match the tree layout and see the match/unmatched counts without locking in bad results. The completion banner reads "Import complete (DRY RUN - no changes written to state DB)" so operators can't accidentally miss that nothing was persisted.
