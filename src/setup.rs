...
pub(crate) fn run_setup(config_path: &Path) -> anyhow::Result<SetupResult> {
    ...
    println!();
    println!("Welcome to icloudpd-rs setup!");
    println!();
...
fn ask_what_to_download(answers: &mut SetupAnswers) -> anyhow::Result<()> {
    ...
    if scope == 1 {
        ...
        if !answers.albums.is_empty() {
            println!("  Tip: run `icloudpd-rs sync --list-albums` to see available album names.");
        }
    }
...
fn ask_running_mode(answers: &mut SetupAnswers) -> anyhow::Result<()> {
    ...
    let mode = Select::new()
        .with_prompt("How will you run icloudpd-rs?")
        .items(&mode_items)
        .default(0)
        .interact()?;
...
fn ask_extras(answers: &mut SetupAnswers) -> anyhow::Result<()> {
    ...
    let cookie: String = Input::new()
        ...
        .default("~/.icloudpd-rs".to_string())
        .interact_text()?;
    if cookie != "~/.icloudpd-rs" {
...
fn generate_toml(answers: &SetupAnswers) -> String {
    ...
    writeln!(out, "# icloudpd-rs configuration").ok();
    writeln!(out, "# Generated by: icloudpd-rs setup").ok();
    writeln!(out).ok();
    ...
    match &answers.cookie_directory {
        ...
        None => writeln!(out, "# cookie_directory = \"~/.icloudpd-rs\"").ok(),
    };
    ...
    writeln!(out, "# temp_suffix = \".icloudpd-tmp\"").ok();
